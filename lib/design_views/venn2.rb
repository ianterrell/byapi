module DesignViews
  class Venn2 < Base
    def xml(markup, parameters, options={})
      width = (options[:width] || 100).to_i
      height = (options[:height] || 100).to_i
      ...
      svg_image :xml => markup, :height => height+2*bleed_y, :width => width+2*bleed_x do |svg|
        svg.g :transform => "translate(#{bleed_x} #{bleed_y})" do |g|
