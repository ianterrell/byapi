  ...
  class Venn2 < Base
    def xml(markup, parameters, options={})
      ...
      svg_image :xml => markup, :height => options[:height], :width => options[:width] do |svg|
        ...
        svg.circle :cx => first_center[:x], :cy => first_center[:y], :r => radius, :fill => "#0000ff", :"fill-opacity" => "0.5"
        ...
        svg.circle :cx => second_center[:x], :cy => second_center[:y], :r => radius, :fill => "#ff0000", :"fill-opacity" => "0.5"
