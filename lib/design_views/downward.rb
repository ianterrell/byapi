  ...
  class Downward < Base
    def xml(markup, parameters, options={})
      ...
      svg_image :xml => markup, :height => height, :width => width do |svg|
        ...
        # Downward slope!
        svg.path :d => "M#{2*axes_offset} #{1.5*axes_offset} S#{curve*axes_offset} #{height-curve*axes_offset} #{width-1.5*axes_offset} #{height-1.5*axes_offset}", :stroke => font_color, :"stroke-width" => arrow_size, :fill => "none"
